...
export class ArcTread extends Tread {
  ...
  rebuildByParent({vPos, vIndex, vEndLVec}) {
    ...
    this.inRadius = this.parent.radius
    this.center = this.parent.center
    this.outRadius = this.parent.radius - this.stepLength
    ...
    this.startLVec = new UtilVec2(this.endLVec).round(angle)
  ...
  createArcBorder() {
    ...
    let treadInStartVec = this.getTreadStartVec(this.inRadius)
    let treadOutStartVec = this.getTreadStartVec(this.outRadius)
  ...
  createArcOutline (vOutStartVec, vInStartVec, vOutStartAngle, vInStartAngle) {
    ...
    let p2 = new Edge().setByVec(this.center, this.endLVec, this.outRadius).p2
    let p3 = new Edge().setByVec(this.center, vOutStartVec, this.outRadius).p2
    let p4 = new Edge().setByVec(this.center, vInStartVec, this.inRadius).p2
